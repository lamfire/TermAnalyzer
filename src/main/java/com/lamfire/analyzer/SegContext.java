...
public class SegContext {
     ...
     */
    private Set<Seg> buffLocker;
    /*
     * 词元结果集，为每次游标的移动，存储切分出来的词元
     */
	private SortedLinkSet lexemeSet;

    
    public SegContext(char[] segmentBuff, boolean isMaxWordLength){
    	this.isMaxWordLength = isMaxWordLength;
    	this.segmentBuff = segmentBuff;
    	this.buffLocker = new HashSet<Seg>(4);
    	this.lexemeSet = new SortedLinkSet();
	}
	...
	public void lockBuffer(Seg seg){
		this.buffLocker.add(seg);
	}
	
	public void unlockBuffer(Seg seg){
		this.buffLocker.remove(seg);
	}
	...
	private class SortedLinkSet{
		...
		private void addLexeme(Lexeme lexeme){
			...
			if(lexeme.getLexemeType() == Lexeme.TYPE_CJK_SIFFIX){//词元为后缀补充到上一个词元中
				this.tail.setLength(this.tail.getLength() + lexeme.getLength());
				return;
			}
			...
			if(this.head.compareTo(lexeme) > 0){//词元接入链表头部
			...
			if(l.compareTo(lexeme) < 0){//词元插入链表中的某个位置
				lexeme.setPrev(l);
