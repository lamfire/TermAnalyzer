 ...
 *
 */
public class LetterSeg implements Seg {
...
//		bufferLockFlag = this.processPureArabic(input, context) || bufferLockFlag;
	 ...
	 * @return
	 */
	private boolean processMixLetter(char input , SegContext context){
...
//	 * @return
//	 */
//	private boolean processPureArabic(char input , SegContext context){
//		boolean needLock = false;
//		
...
//	}
	 ...
	 * @return
	 */
	private boolean processEnglishLetter(char input , SegContext context){
		boolean needLock = false;
		
		if(letterStart == -1){//当前的分词器尚未开始处理数字字符	
			if(CharHelper.isEnglishLetter(input)){
				//记录起始指针的位置,标明分词器进入处理状态
	...
	private boolean isAcceptedCharStart(char input){
		return CharHelper.isEnglishLetter(input)
				|| CharHelper.isArabicNumber(input);
	}
	...
	private boolean isAcceptedChar(char input){
		return isLetterConnector(input) 
				|| CharHelper.isEnglishLetter(input)
				|| CharHelper.isArabicNumber(input);
	}
